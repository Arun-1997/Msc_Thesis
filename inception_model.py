...
def set_config():
    ...
    wandb.init(project="test-project", entity="msc-thesis",config=config)
    now = datetime.now()
...
#         include_top=include_top)
    ...
    if include_top:
        ...
        # x = Dense(classes, activation='softmax', name='predictions')(x) # Change it here so that the final layer will be linear activation for 1 node
...
if __name__ == '__main__':
    ...
    print(model.summary())
    visualkeras.layered_view(model, to_file="Inception_model_architecture.png",legend=True,scale_z=0.020,scale_xy=1)
