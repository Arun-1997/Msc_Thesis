...
training_path = "Input/sentinel/test_data_from_drive/patches_all/all_states_normalised_10bands/train"
target_file_path = "Input/Target/concat/target_yield.shp"
patch_dim = (256, 256, 11)



# maptf.keras.losses.mean_absolute_percentage_error
...
"loss_function":
    {'segmentation': 'binary_crossentropy', 
     'reg_output': 'mean_absolute_percentage_error'},
...
wandb.run.name = wandb.run.id+"_10bands_arch4"+date_time
...
def read_training():
    ...
    for file in training_file_list:
        ...
        if np.isnan(patch_src_read).any():
            ...
            continue
        if np.unique(patch_src_read[:,:,10],return_counts=True)[1][1] < 655:
        ...
        if len(query) != 1:
            ...
            continue
        x.append(patch_src_read[:,:,0:10])
        y.append(patch_src_read[:,:,10])
...
def build_unet_model():
    # inputs
    inputs = layers.Input(shape=(256,256,10))
    ...
    # outputs
    # seg_layer = layers.Conv2D(1,1, padding="same",activation = "sigmoid")(u9)
    reg_layer = layers.Conv2D(512, (1, 1), activation='relu')(bottleneck)
    # reg_layer = layers.MaxPooling2D((2, 2))(reg_layer)

    reg_layer = layers.Conv2D(256, (1, 1), activation='relu')(reg_layer)
    # 
    reg_layer = layers.Conv2D(128, (1, 1), activation='relu')(reg_layer)
    reg_layer = layers.MaxPooling2D((2, 2))(reg_layer)
    reg_layer = layers.Conv2D(64, (1, 1), activation='relu')(reg_layer)
    reg_layer = layers.Conv2D(32, (1, 1), activation='relu')(reg_layer)
    reg_layer = layers.MaxPooling2D((2, 2))(reg_layer)
    reg_layer = layers.Flatten()(reg_layer)
    reg_layer = BatchNormalization()(reg_layer)
    reg_layer = layers.Dense(256, activation='relu')(reg_layer)
    reg_layer = layers.Dense(128, activation='relu')(reg_layer)
    reg_layer = layers.Dense(64, activation='relu')(reg_layer) # Add another dense layer
    # reg_layer = Dropout(0.3)(reg_layer)
...
def generate_data(x, y,y_reg, batch_size = 32):
    ...
    for j in range(num_of_batches):
        ...
        x_batch = np.zeros((b_size, 256, 256, 10))
...
callbacks = [                             ModelCheckpoint("unet_multi_head_output/model_10bands_BSize_"+str(config["batch_size"])+"_NEpochs_"+str(config["epochs"])+"_"+str(datetime.now())+".h5",save_freq='epoch'),
ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=2),
CSVLogger("unet_multi_head_output/data_10bands_BSize_"+str(config["batch_size"])+"_NEpochs_"+str(config["epochs"])+"_"+str(datetime.now())+".csv"),
    # TensorBoard(),
...
model_history = unet_model.fit(X_train,[y_train,y_reg_train],
                              validation_data = (X_test,[y_test,y_reg_test]),
